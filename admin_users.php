...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_selected'])) {
        ...
        if (!empty($selected_rows)) {
            foreach ($selected_rows as $row_id) {
                $query = "UPDATE users SET 
                    username = '{$selected_rows[$row_id]['username']}', 
                    email = '{$selected_rows[$row_id]['email']}', 
                    password = '{$selected_rows[$row_id]['password']}', 
                    role = '{$selected_rows[$row_id]['role']}' 
                    WHERE user_id = '{$selected_rows[$row_id]['user_id']}'";
        ...
        if (!empty($selected_rows)) {
            foreach ($selected_rows as $row_id) {
                $query = "DELETE FROM users WHERE user_id = '$row_id'";
                mysqli_query($connection, $query);
            }
        }
    }
														...
														<?php foreach ($row as $key => $value) { ?>
																<td>
																		...
																		<?php } elseif ($key === 'password') { ?>
																				<input type="password" name="selected_rows[<?= $row['user_id']; ?>][<?= $key; ?>]" value="">
																		<?php } else { ?>
																				<input type="text" name="selected_rows[<?= $row['user_id']; ?>][<?= $key; ?>]" value="<?= $value; ?>">
																		<?php } ?>
																</td>
														<?php } ?>
														<td><button type="submit" name="update_selected">Update</button></td>
														<td><button type="submit" name="delete_selected">Delete</button></td>
														<td><input type="checkbox" name="selected_rows[]" value="<?= $row['user_id']; ?>"></td>
												</tr>
										<?php } ?>
								</table>
